package com.jlrm.springbootwebflux.client.handler;
...
import com.jlrm.springbootwebflux.client.service.ProductoService;
...
public class ProductoHandler {
	...
	public Mono<ServerResponse> ver(ServerRequest request) {
		...
		return errorHandler(service.findById(id).flatMap(p -> 
			ServerResponse.ok()
	...
	public Mono<ServerResponse> editar(ServerRequest request) {
		...
		return errorHandler(producto.flatMap(p -> service.editar(p, id)).flatMap(p -> ServerResponse
	...
	public Mono<ServerResponse> upload(ServerRequest request) {
		...
		return errorHandler(request.multipartData().map(multipart -> multipart.toSingleValueMap().get("file"))
				.cast(FilePart.class).flatMap(file -> service.upload(file, id)).flatMap(p -> ServerResponse
						.created(URI.create("/api/client/".concat(id))).contentType(APPLICATION_JSON).bodyValue(p)));
	}
	...
	private Mono<ServerResponse> errorHandler(Mono<ServerResponse> response) {
		return response.onErrorResume(error -> {
			...
			if (errorResponse.getStatusCode() == HttpStatus.NOT_FOUND) {
				...
				body.put("error", "No existe el producto");
